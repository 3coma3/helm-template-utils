...
- Format a key, no prefix is added to variable names
  {{ include "toEnv" targetKey }}

- Format a key and add a custom prefix to variable names
  {{ include "toEnv" (list targetKey "prefix") }}

- Format a key and use its name as prefix
  {{ include "toEnv" (list parentKey "targetSubkey") }}
...
{{- if kindIs "slice" . -}}
  ...
  {{- $prefix = (printf "%s_" (include "SSCase" $key)) }}
{{- else if kindIs "map" . -}}
  {{- $target = . -}}
{{- end -}}
...
- name: {{ (eq $k "") | ternary (trunc -1 $prefix) (printf "%s%s" $prefix (include "SSCase" $k)) }}
